...
/**
 * Transformer: FNBO Dynamic Media / Scene7 images.
 *
 * Converts DM/Scene7 <img src> tags into anchors that survive the
 * docx -> markdown round-trip. A companion auto-block in scripts/scripts.js
 * (installed by the site-migration orchestrator) rebuilds them into
 * responsive <picture> elements at render time.
 *
 * Detection: Scene7 IS/Image URLs (path starts with /is/image/) confirmed in
 * both migration-work/metadata.json and migration-work-product/metadata.json
 * .images.mapping keys (e.g. https://s7d1.scene7.com/is/image/fnnistage/...).
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into block cells (cards/carousel/etc.). Rewriting imgs to
 * anchors earlier would leave parsers with empty image cells.
 *
 * Canonical helpers copied byte-identical from references/dm-scene7-helpers.js
 * (the subset the server-side transformer consumes).
 */
