...
export default async function decorate(block) {
  ...
  // If the first row is a heading-only row (no quote paragraph), lift it out
  // as a persistent carousel title rendered above the slides (matches source).
  let titleEl = null;
  if (allRows.length > 1) {
    const firstRow = allRows[0];
    const cells = firstRow.querySelectorAll(':scope > div');
    const contentCell = cells[cells.length - 1];
    const heading = contentCell && contentCell.querySelector('h1, h2, h3, h4, h5, h6');
    if (heading && !contentCell.querySelector('p')) {
      titleEl = document.createElement('div');
      titleEl.classList.add('carousel-quote-title');
      titleEl.append(heading);
      firstRow.remove();
      allRows.shift();
    }
  }
